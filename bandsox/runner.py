...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="namespace",
        choices=["namespace", "none"],
        help="Vsock isolation mode: 'namespace' (default) or 'none'",
    )
    parser.add_argument(
        "--vsock-config",
        type=str,
        help="JSON-encoded vsock config for restored VMs (uds_path, port, cid)",
    )
    parser.add_argument(
        "--vsock-dir",
        type=str,
        help="Directory for vsock sockets",
    )
    ...
    vm = MicroVM(
        ...
        args.socket_path,
        netns=args.netns,
        vsock_isolation=args.vsock_isolation,
        vsock_dir=args.vsock_dir,
    )

    # Parse vsock config if provided (for restored VMs)
    vsock_config = None
    if args.vsock_config:
        try:
            vsock_config = json.loads(args.vsock_config)
            logger.info(f"Vsock config provided: {vsock_config}")
        except json.JSONDecodeError as e:
            logger.warning(f"Failed to parse vsock config: {e}")
    ...
    try:
        ...
        # Set up vsock listener if config was provided (for restored VMs)
        # This runs in the runner process so the listener stays alive as long as the VM runs
        if vsock_config and vsock_config.get("enabled"):
            try:
                vsock_port = vsock_config.get("port", 9000)
                vsock_uds_path = vsock_config.get("uds_path")

                if vsock_uds_path:
                    vm.vsock_socket_path = vsock_uds_path
                    ...
                    logger.info(
                        f"Vsock listener started: port={vsock_port}, path={vsock_uds_path}"
                    )
                else:
                    logger.warning(
                        "Vsock config missing uds_path, skipping listener setup"
                    )
            except Exception as e:
                logger.warning(f"Failed to setup vsock listener in runner: {e}")
